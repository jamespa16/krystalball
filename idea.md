...
3. Online training loop:
   ...
   - **Adjustable real-frame interval (replay buffer):** a live on-screen slider sets how far training is allowed to lag behind live video, from 0 (train on every frame immediately, the original 1:1 behavior) up to 30-60 seconds. Above 0, incoming real frames are appended to a small in-memory FIFO replay buffer instead of being trained on right away; a separate consumption step drains at most one frame per loop iteration once the backlog exceeds the target delay (converted from seconds to frames via the current FPS). This means training lags real time by roughly the slider's interval, but it still eventually trains on *every* real frame — nothing is discarded, unlike a pure free-run/self-feed scheme. (If the slider is lowered mid-run, the buffer drains faster than it fills — more than one frame per iteration — until the backlog catches back down to the new, smaller target.)
...
- Real-frame interval in seconds (default 0 = every frame is real, i.e. the original 1:1 behavior), adjustable live via an in-window OpenCV trackbar. Converted internally to a frame count using the current smoothed FPS.
